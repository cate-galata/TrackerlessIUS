...
class BlockFinalImg(nn.Module):
    def __init__(self, n_channels: int=3, nfeat_finalblock: int=3, n_out: int=64, last_act: str='tanh', nb_blocks: int=6) -> None:
        super().__init__()

        if nfeat_finalblock==n_channels:
            self.res = []
        else:
            self.res = [nn.Conv2d(in_channels=n_channels, out_channels=nfeat_finalblock, kernel_size=1, padding=0, padding_mode='reflect')]
            
        for _ in range(nb_blocks):
            self.res.append(ResnetBlock(nfeat_finalblock, padding_type='reflect', norm_layer=nn.InstanceNorm2d, use_dropout=False, use_bias=True))
        self.res = nn.Sequential(*self.res)
        
    
        self.convt2 = nn.Conv2d(in_channels=nfeat_finalblock, out_channels=nfeat_finalblock, kernel_size=7, padding=3, padding_mode='reflect')
        self.norm2 = nn.InstanceNorm2d(nfeat_finalblock)
        self.act = nn.LeakyReLU(0.2, True)
        self.convt3 = nn.Conv2d(in_channels=nfeat_finalblock, out_channels=n_out, kernel_size=7, padding=3, padding_mode='reflect')
        if last_act=='tanh':
