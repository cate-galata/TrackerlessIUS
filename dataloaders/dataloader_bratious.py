...
def get_training_loader(train_files, batch_size, num_workers, use_spacial_augmentation=True, cache=False):
    ...
    train_transforms = Compose(
        [
            ...
            rand_affine,  # 1
            ResizeWithPadOrCropd(keys=all_keys, spatial_size=input_size),
            rand_gauss_noise,  # 2
            rand_gauss_smooth,  # 3
            scale_intensity,  # 4
            shift_intensity,  # 5
            ...
            adjust_contrast,  # 8
            mirror_x, mirror_y, # 9
            EnsureTyped(keys=modality_keys, data_type="tensor", track_meta=False),
        ], 
        unpack_items=True)
    ...
    train_loader = DataLoader(volume_ds, batch_size=batch_size, num_workers=num_workers, shuffle=True)
...
def get_validation_loader(val_files, batch_size, num_workers, cache=False):
    ...
    val_transforms = Compose(
        [
            ...
            ResizeWithPadOrCropd(keys=["img", "seg"], spatial_size=input_size), 
    ...
    # use batch_size=1 to check the volumes because the input volumes have different shapes
    val_loader = DataLoader(volume_ds, batch_size=batch_size, num_workers=num_workers)
