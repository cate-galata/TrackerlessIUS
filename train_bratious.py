...
from utilities.dataloader_bratious import get_training_loader, get_validation_loader
...
def train(paths_dict, model, criterion, soft_criterion, device, save_path, logger, opt):
    ...
    folder_case = os.path.join(opt.model_dir, "bratious", str(opt.learning_rate), str(opt.spacial), opt.comment)
    ...
    df = pd.DataFrame(columns=["epoch", "wall_time", "val_dice", "best_epoch", "best_val", "lr"])
    df_test = pd.DataFrame(columns=["epoch", "wall_time", "test_dice"])
    df_train = pd.DataFrame(columns=["epoch", "wall_time", "train_dice"])
    best_val = None
    best_epoch = 0
    epoch = 0
    ...
    while continue_training:
        ...
        for phase in PHASES[::-1]:
            ...
            # Validation bookkeeping: best model + lr schedule + CSV logging
            if phase == "validation":
...
def main():
    ...
    nb_images = len(list_file)
    nb_val = int(len(list_file) * 0.2)
    nb_test = int(len(list_file) * 0.1)

    def add_subject(split, subject):
        img_path = os.path.join(opt.path_data, subject + ".nii.gz")
        lab_path = os.path.join(opt.path_labels, subject + ".nii.gz")

        if os.path.exists(img_path) and os.path.exists(lab_path) and nib.load(img_path).get_fdata().ndim < 3 and nib.load(lab_path).get_fdata().ndim < 3:
            paths_dict[split].append({"img": img_path, "seg": lab_path})

    for subject in list_file[: nb_images - nb_val - nb_test]:
        add_subject("training", subject)

    for subject in list_file[nb_images - nb_val - nb_test : nb_images - nb_test]:
        add_subject("validation", subject)

    for subject in list_file[nb_images - nb_test : ]:
