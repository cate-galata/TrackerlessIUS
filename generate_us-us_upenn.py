...
path_output = '/lustre/fsn1/projects/rech/jkq/ubt15jc/synthetic_us-us/{}/{}/{}'
...
for folder in tqdm(os.listdir(path_data)):
    ...
    if os.path.exists(os.path.join("/lustre/fsn1/projects/rech/jkq/ubt15jc/synthetic_us-us", folder)) and len(os.listdir(os.path.join("/lustre/fsn1/projects/rech/jkq/ubt15jc/synthetic_us-us", folder, 'data_mri'))) >= 24:
    ...
    while count < NB_cases:
        ...
        Ps = walk_on_surface_bidirectional(
            filtered_arr,
            x0_world=P_0,
            n_world=sweep_dir,
            dx_mm=0.5,
            n_steps=len(sample_Ps),
            len_path_max_mm=np.abs(tumor_max),
            len_path_min_mm=np.abs(tumor_min),
            radius_mm=radius_mm,
            # max_jump_mm=50.0,   # safety: stop if projection is weird
        )
        d_vecs = [d_vec_0 for _ in range(Ps.shape[0])]
        i_vecs = [i_vec_0 for _ in range(Ps.shape[0])]
        ...
        for modalities_set in total_subsets_mr:
            ...
            P_us_torch = torch.from_numpy(P_us).to(device)
